...
class UserInfoListPro extends StatefulWidget {
  ...
  UserInfoListPro(this.isPro, this.excludeUid, this.uid);
...
class _UserInfoListPro extends State<UserInfoListPro> {
  ...
  Widget build(BuildContext context) {
    ...
    List<UserInfo> proUserInfo = _getUserInfoList(userInfo, widget.isPro, widget.excludeUid, widget.uid,);

    return ListView.builder(
      itemCount: proUserInfo.length,
      itemBuilder: (context, index) {
        return UserInfoTile(userInfo: proUserInfo[index]);
      },
    );
  }
}

List<UserInfo> _getUserInfoList(List<UserInfo> p, bool isPro, bool excludeUid, String uid) {
